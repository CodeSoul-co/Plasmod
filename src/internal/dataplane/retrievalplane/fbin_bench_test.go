...
// TestBenchFbinDiskANNLoop runs Microsoft DiskANN on the same fbin dataset
// using the single-query loop methodology. DiskANN is on-disk: Build writes
// the vamana graph + PQ shards + raw data to a temp prefix, Search reads
// from the SSD via beam-search.
//
// Tuning (search_list=128, max_degree=48, beamwidth=8) uses Knowhere
// defaults; recall@10 should approach 1.0 since disk_pq_dims=0 (uncompressed
// raw vectors stored on SSD).
func TestBenchFbinDiskANNLoop(t *testing.T) {
	const topK = 10
	root, fbinPath := locateFbin(t)
	t.Logf("data: %s", fbinPath)
	...
	// DiskANN needs a writable, non-colliding prefix.
	tmpDir, err := os.MkdirTemp("", "plasmod_diskann_bench_*")
	if err != nil {
		t.Fatalf("mkdtemp: %v", err)
	}
	defer os.RemoveAll(tmpDir)
	prefix := filepath.Join(tmpDir, "idx")
	t.Logf("diskann prefix: %s", prefix)

	r, err := NewDiskANNRetriever(dim, "L2", prefix)
	if err != nil {
		t.Skipf("DISKANN not available in this build: %v", err)
	}
	defer r.Close()

	t.Logf("building DISKANN (n=%d, dim=%d, R=48 L=128 default)...", n, dim)
	t0 := time.Now()
	if err := r.Build(vecs, n); err != nil {
		t.Fatalf("Build: %v", err)
	}
	...
	// Warm-up — first DiskANN searches incur PQ-cache page-ins.
	for i := 0; i < 64; i++ {
		_, _, _ = r.Search(vecs[i*dim:(i+1)*dim], topK, nil)
	}
	...
	latMS := make([]float64, n)
	hits := 0
	t.Logf("running %d single-query searches (topK=%d, beamwidth=8 default)...", n, topK)
	t0 = time.Now()
	for q := 0; q < n; q++ {
		qv := vecs[q*dim : (q+1)*dim]
		ts := time.Now()
	...
	fmt.Println("\n========== plasmod DISKANN (LOOP) ==================")
	fmt.Printf("dataset      : TEST/testQuery10K.fbin (n=%d, dim=%d, self-query)\n", n, dim)
	fmt.Printf("params       : R=48  L_build=128  beamwidth=8  topK=%d  metric=L2 (C++ defaults)\n", topK)
	fmt.Printf("build_s      : %.3f\n", buildSec)
	fmt.Printf("QPS          : %.1f\n", qps)
	fmt.Printf("p50_ms       : %.4f\n", percentile(latMS, 50))
	fmt.Printf("p95_ms       : %.4f\n", percentile(latMS, 95))
	...
	out := filepath.Join(root, "TEST", "metrics_plasmod_diskann_loop.json")
	bs, _ := json.MarshalIndent(map[string]any{
		"engine":       "plasmod DISKANN (loop) via CGO",
		"dataset":      "TEST/testQuery10K.fbin (self-query)",
		"build_s":      round3(buildSec),
		"wall_s":       round3(wallSec),
		"qps":          round1(qps),
		...
		"recall_at_10": round4(recall),
		"params": map[string]any{
			"index": "DISKANN", "max_degree": 48, "search_list_build": 128, "beamwidth": 8,
			"nb": n, "nq": n, "dim": dim, "topK": topK, "metric": "L2",
		},
	}, "", "  ")
	_ = os.WriteFile(out, bs, 0644)
