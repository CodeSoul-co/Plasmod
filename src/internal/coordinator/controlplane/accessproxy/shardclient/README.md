...
- **Access Proxy** (`internal/coordinator/controlplane/accessproxy/`): Uses `shardclient` to route search/query requests to query runtimes
- **Query Control** (`internal/coordinator/controlplane/querycontrol/`): Provides shard leader information via `GetShardLeaders` RPC
- **Query Runtime** (`internal/dataplane/retrievalplane/queryruntime/`): Receives and processes requests routed by `shardclient`
