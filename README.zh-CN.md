...
- Go 服务（[`src/cmd/server/main.go`](src/cmd/server/main.go)），包含 14 条 HTTP 路由，通过 `context.WithCancel` 优雅关闭
- 管理员数据集清理：`POST /v1/admin/dataset/delete` 软删除内容匹配指定选择器的 **Memory** 记录（**AND** 语义）。**`workspace_id` 必填**，`file_name`、`dataset_name`、`prefix` 至少填一个。`dry_run` 仅报告匹配结果，不执行变更。
  - 匹配规则：`file_name` 设置时，内容须包含 `dataset=<file_name>`；`dataset_name` 设置时须包含 `dataset_name:<dataset_name>`；`prefix` 设置时须包含 `dataset=<prefix`（在 `dataset=` 标记上做前缀子串匹配）。
  - 响应字段：`matched`、`deleted`、`memory_ids`（在 `dry_run` 模式下 `deleted` 保持 `0`，但 `memory_ids` 仍列出所有匹配项）。
- 管理员数据集**硬删除（purge）**：`POST /v1/admin/dataset/purge`，使用相同选择器和必填 **`workspace_id`**。物理删除热/温/冷层记录、温层图边、冷层嵌入向量与冷层 Memory 对象。默认 `only_if_inactive=true`（仅清除已软删除/非活跃的记忆）；设为 `false` 可同时清除活跃匹配项。每次成功清除会追加一条不可变 `AuditRecord`（`reason_code=dataset_purge`）。
...
- [`src/internal/access`](src/internal/access)：HTTP 网关，14 条路由含摄入、查询和规范 CRUD
...
- Go HTTP API（14 条路由）、Python SDK 和集成测试套件 ✅
