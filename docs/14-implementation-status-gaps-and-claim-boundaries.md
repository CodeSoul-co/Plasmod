...
| `GET/POST /v1/agents` | canonical handler/ObjectCoordinator | none | Canonical Graph | POST 写 version snapshot；绕过 Event/WAL/retrieval/access gate |
| `GET/POST /v1/sessions` | canonical handler/ObjectCoordinator | none | Canonical Graph | 同上 |
| `GET/POST /v1/memory` | canonical handler/ObjectCoordinator | none | Canonical Graph | POST 写 version snapshot，但不写 WAL/retrieval projection |
| `GET/POST /v1/states` | canonical handler/ObjectCoordinator | none | Canonical Graph | POST 写 version snapshot；不提供 Event replay/monotonic state guard |
| `GET/POST /v1/artifacts` | canonical handler/ObjectCoordinator | none | Canonical Graph | POST 写 version snapshot；不写 WAL/retrieval projection |
...
| `GET/POST /v1/share-contracts` | canonical handler | none | Governance, Canonical Graph | contract storage；query/reference share 强制，其他 write path 部分 |
...
| `/v1/internal/memory/share` | `DispatchShareWithContract`/`DispatchShare` | Collaboration + Governance + Ingest | owner/contract 校验后写 derived Event/WAL、shared Memory、Version、Edge 和 retrieval projection |
...
| evidence-bearing, access-filtered query response | planner -> retrieval -> canonical access gate -> assembler -> graph endpoint filter | evidence, semantic, runtime access/e2e query tests | response objects 是 IDs；caller identity 仍需外部认证绑定 |
...
| contract-backed derived sharing | ShareContractStore + `DispatchShareWithContract` + Ingest Chain | semantic policy and runtime share/access tests | share 强制 derive/read；raw CRUD/lifecycle/conflict 未统一 write ACL |
...
当前 `projectWALEntry` 实际顺序是 canonical projection -> retrieval ingest -> hot promotion/conflict/precompute/worker dispatch。canonical-first 保留可恢复事实；若 retrieval 失败，controller 不推进 visible watermark，query 以 `MutationLSN`/`ReadWatermarkLSN` 隐藏未完成 projection。retrieval 与 canonical store 仍不属于同一跨系统事务。
...
| Canonical projection | Event/Memory/stable State/可选 Artifact/edge/version transaction，随后 retrieval ingest；hot promotion | tool/consolidation/reflection 与专用 State checkpoint via subscriber |
...
| Canonical commit succeeds, retrieval ingest fails | authoritative object exists but LSN is not visible | projection error/tracker retry；query watermark gate | retry same WAL LSN/reindex；no automatic cross-plane rollback |
...
| Communication | source/target agent/memory ID + optional contract | shared memory ID | Runtime path derives Event/WAL/Memory/Version/Edge; legacy worker copies scoped Memory |

### 14.5.6. ID and Version Invariants

| Record | 默认规则 |
|---|---|
| Memory | 默认 `mem_<event_id>`；typed Memory 可使用显式 object ID |
| State | `state_<24-hex sha256 prefix of tenant/workspace/agent/session/state_key>`；默认 key 是 `last_memory_id` |
...
| Version | Memory/Artifact 保持对象版本；State 由 store history 单调递增；均关联 mutation event/LSN 并保存 snapshot |
