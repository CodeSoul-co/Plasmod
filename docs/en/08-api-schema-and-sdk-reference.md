...
| `403` | Share owner or contract scope/read/derive policy denied the operation |
| `404` | Share source Memory or named ShareContract does not exist |
...
- 401/403: correct credentials, ownership, or contract authorization; do not retry unchanged.
- 404: verify the object/contract ID and scope.
...
Projection retry for the same WAL LSN reuses the canonical mutation, and replaying an old State mutation does not roll current State backward. Submitting the same event ID under a new LSN returns a duplicate result with the original LSN and does not append another object version. WAL append, projection, and direct CRUD still have different duplicate semantics; do not generalize from one route to every route.
...
The active materializers do not yet treat `enabled` and `targets` as universal hard gates. Every Event ingest produces a Memory, Memory ObjectVersion, and one stable State: an explicit State key is updated when present; otherwise the scope's `last_memory_id` State is updated. Artifact creation remains conditional on event/object/payload signals. A target does not guarantee automatic creation of an unknown type, and `enabled=false` does not guarantee that default canonical projection is skipped.

Memory IDs default to `mem_<event_id>`; only a typed Memory Event may override this with an explicit `object.object_id`. State IDs are generated by `CanonicalStateID(tenant_id, workspace_id, agent_id, session_id, state_key)` as a 24-hex digest. The same scope/key is stable across restart and replay, while different scopes do not collide. Artifact IDs prefer `object.object_id`. These rules are part of the persistence compatibility surface; changes require migration and replay validation.

Memory, State, Artifact, Edge, and ObjectVersion share these persisted fields:

| Field/type | Semantics |
|---|---|
| `mutation_lsn` | WAL LSN that produced the canonical mutation |
| `CanonicalAccess.tenant/workspace/team/session` | structured scope |
| `CanonicalAccess.owner_agent_id` | owner identity |
| `CanonicalAccess.visibility` | visibility mode |
| `visible_to_agents`, `visible_to_roles` | explicit read grants |
| `policy_tags`, `share_contract_id` | governance labels and contract binding |

Memory and Artifact also record `materialized_at`. ObjectVersion `snapshot` contains recoverable object content rather than only a version label.
...
- scope: `tenant_id`, `workspace_id`, `team_id`, `agent_id`, `session_id`;
- principal: `requester_agent_id`, `requester_roles`; `agent_id` remains a selector and is only a compatibility fallback when requester identity is omitted;
...
Responses include objects/nodes, edges, provenance, versions, applied filters, proof traces, chain traces, evidence-cache state, retrieval summaries, query status, and hints. `access_decisions` explains why returned objects were allowed; `read_watermark_lsn` is the visibility frontier used for access filtering. Denied objects receive no decision, avoiding existence disclosure.
...
`/v1/query/batch` is a projection-only native route and does not execute the canonical principal/access/evidence contract above. Agent requests that require object-level governance must use `/v1/query`.
