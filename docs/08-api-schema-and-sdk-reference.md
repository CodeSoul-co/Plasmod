...
| `401` | admin key 缺失或错误 |
| `403` | share owner/contract scope/read/derive policy 拒绝 |
| `404` | share source Memory 或指定 ShareContract 不存在 |
| `405` | Method 不支持 |
...
- 400/405：不重试，修正调用；
- 401/403：修正凭证、owner 或 contract 授权，不应原样重试；
- 404：确认 object/contract ID 与 scope；
...
同一 WAL LSN 的 projection retry 复用 canonical mutation；旧 State mutation replay 不回滚当前值。相同 event ID
如果以新 LSN 再次提交，Runtime 返回 original LSN 的 duplicate 结果且不追加对象版本。WAL append、projection
和直接 CRUD 的重复行为仍不同，不能从一个 route 的幂等性外推到所有 route。
...
Memory ObjectVersion 和一个 stable State：有 state key 时写该 key；否则更新当前 scope 的
`last_memory_id`。Artifact 再由 event/object/payload 条件触发。设置 target 不保证任意未知类型能被自动创建，
设置 `enabled=false` 也不保证跳过上述默认 canonical projection。

Memory ID 默认 `mem_<event_id>`；只有 typed Memory Event 的显式 `object.object_id` 可覆盖该规则。State ID
统一由 `CanonicalStateID(tenant_id, workspace_id, agent_id, session_id, state_key)` 生成 24-hex digest，因而同一
scope/key 在重启和 replay 后保持稳定，跨 scope 不碰撞。Artifact 可优先采用 `object.object_id`。这些规则属于
持久化兼容面，修改时需要迁移与 replay 验证。

Memory、State、Artifact、Edge、ObjectVersion 共享以下持久字段：

| Field/type | Semantics |
|---|---|
| `mutation_lsn` | 产生该 canonical mutation 的 WAL LSN |
| `CanonicalAccess.tenant/workspace/team/session` | 结构化 scope |
| `CanonicalAccess.owner_agent_id` | owner identity |
| `CanonicalAccess.visibility` | scope mode |
| `visible_to_agents`, `visible_to_roles` | 显式 read grants |
| `policy_tags`, `share_contract_id` | 治理标签与协议绑定 |

Memory/Artifact 还记录 `materialized_at`；ObjectVersion 的 `snapshot` 保存可恢复对象内容，而非仅保存版本标签。
...
- principal：`requester_agent_id`、`requester_roles`；`agent_id` 仍是查询 selector，并在旧客户端未提供 requester 时作为兼容 fallback；
...
cache、retrieval summary、query status 和 hint。`access_decisions` 只解释已返回对象的允许原因；
`read_watermark_lsn` 是本次访问过滤使用的可见前沿。拒绝对象不返回 decision，避免披露其存在。
...
`/v1/query/batch` 是 projection-only native route，不执行上述 canonical principal/access/evidence contract；
需要对象级治理语义的 agent 请求必须使用 `/v1/query`。
