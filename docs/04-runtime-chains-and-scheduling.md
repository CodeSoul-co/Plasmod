...
| 4 | materialize in memory | WALEntry.Event | IngestRecord、Memory、stable State、可选 Artifact、Edge、Version | projection worker |
| 5 | canonical commit | Event/Memory/State/可选 Artifact/Edges/Versions | store transaction/upserts | authoritative write |
...
重要事实：`Runtime.projectWALEntry` 现在将 `MaterializationResult.State` 与版本历史放入同一 `CanonicalProjection`。显式 `state_key/state_value` 更新稳定 keyed State；其他 Event 更新该 scope 的 `last_memory_id` 状态。Runtime 在 commit 前读取现有 State/Version、分配单调版本并关闭旧版本。异步 `StateMaterializationWorker` 仍用于专用 apply/checkpoint chain，但不是主写链 State 可见性的唯一来源。

canonical commit 先于 retrieval ingest。若 retrieval 写失败，controller 不推进 visible watermark；同一 WAL LSN 重试时会幂等补全 retrieval projection。Query 还会拒绝 `MutationLSN` 超过当前 `ReadWatermarkLSN` 的 canonical object，因此 canonical 已落盘不等于请求已可见。
...
| Event signal | Output |
|---|---|
| every accepted Event | 默认 `mem_<event_id>`（typed Memory 可显式 object ID）、Memory version、stable `last_memory_id` State、base/causal edges、retrieval record |
| artifact/tool-like | Artifact + version + artifact edges；worker 还可能生成 tool trace |
| payload with state key | `CanonicalStateID(tenant, workspace, agent, session, state_key)`；主 canonical commit 中写入 |
...
| Canonical | Event、Memory、State、optional Artifact、Edge、完整 ObjectVersion snapshot；tool trace 仍可异步派生 |
| Projection | canonical commit 后 DataPlane ingest、dirty flush flag |
...
| share/broadcast | from/to agent + Memory ID + optional contract ID | shared Memory ID | derived Event 经 WAL 创建 Memory/Version/Edge/projection |
...
| Share contract storage | canonical CRUD；typed agent/role 与 legacy ACL 字段 |
| Read ACL | `/v1/query` 对 canonical candidate 强制 access evaluator，Evidence graph endpoint 再校验 |
| Write/derive ACL | contract-backed share 强制 source `derive` 和 target `read`；其他 direct mutation 尚未全部统一 |
| Shared object model | 创建 `shared_<source>_to_<target>` 派生 Memory，source 保持 owner，target 获显式 grant |
...
- Canonical：share 路径写 derived Event、shared Memory、完整 ObjectVersion snapshot 和 `derived_from` Edge；ShareContract/Policy 独立存储。
- Access：shared object 带 owner、scope、target agent grant、policy tags 和 contract ID。
- Provenance：share Event 的 source/target causality 生成可遍历 `derived_from` Edge。
- Projection：share 使用 `SubmitIngest`，因此进入普通 retrieval projection 与 watermark 路径。
- Boundary：legacy CommunicationWorker 和 conflict merge 仍有 direct store mutation，未全部具备同一事务/版本/审计语义。

### 04.6.7. Correctness and failure

- Share source missing、owner 不匹配、contract scope/derive/read 不满足时返回 error；same-agent share 保持 no-op。
- Conflict precondition不满足时是 no-op；LWW 相同 version 选择 left。
- WAL-derived share 的 canonical Memory/Edge/Version 在 canonical projection 中提交；retrieval projection 随后完成，audit 尚未并入同一事务。
- Query candidate 与 evidence endpoint 采用 prevention-based access filter；contamination metric 是附加观测，不再替代过滤。
- raw canonical CRUD、legacy direct worker 与全部 lifecycle write 尚未统一使用同一 write policy gate。

### 04.6.8. 声明边界

可声明：显式 ShareContract schema/store、contract-backed WAL-derived sharing、query/evidence canonical access filtering、same-session LWW conflict preservation、handoff/MAS internal adapter。

不可声明：完整 multi-agent transaction、认证身份绑定、所有管理/生命周期写入的全面 ACL、自动 semantic conflict detection、统一 merge policy plugin 或经安全审计的跨 agent zero-leak guarantee。

### 04.6.9. 缺口

1. 将 conflict/merge/handoff 也统一改为 policy-authorized derived Event/WAL；
2. 将 Audit 和 retrieval completion marker 纳入可恢复 share/merge workflow；
3. 对 raw CRUD、lifecycle 和所有 write operation 统一 contract/policy gate；
4. semantic conflict detector 和 pluggable merge strategy；
5. durable microbatch/job status；
6. 认证 principal binding 与 adversarial security tests。
