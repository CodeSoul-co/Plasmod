...
输入是完整 Dynamic Event；输出字段见 [Object and Message Registry](14-implementation-status-gaps-and-claim-boundaries.md)。一个 Event 至少生成 Memory、MemoryVersion、checkpoint State、StateVersion、retrieval record 和基础/因果边；满足 Artifact 规则时还会生成 Artifact 与 ArtifactVersion。原 Event 本身与这些派生结果共同进入 Canonical Projection。
...
- Runtime 将原 Event、Memory、checkpoint State、可选 Artifact、关系和对应版本提交为一个 Canonical Projection；专用 keyed State、tool trace 和生命周期 worker 的输出仍在该事务之外。
