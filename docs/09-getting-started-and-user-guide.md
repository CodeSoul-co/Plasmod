...
状态变化应写成 `state_update`、`state_change` 或相关 tool result Event，并提供 state key/value。主 Runtime
与专用 State materializer 都使用：

```text
state_<sha256-prefix(tenant, workspace, agent, session, state_key)>
```

作为稳定、scope-safe ID。当前版本从 canonical State/ObjectVersion store 读取并递增；同 mutation event 重试不
重复增加版本，旧 event replay 不回滚当前值。

未提供 state key 的普通 Event 会更新该 scope 的稳定 `last_memory_id` State，值指向本次默认 Memory。它不是
每个 event 一个新的 checkpoint ID；因此 latest State 可直接按稳定 ID 读取。`materialization.targets` 当前不是
阻止这一默认 State projection 的硬开关。
...
查询时至少提供 tenant/workspace/agent/session 和 state key，并使用同一 `CanonicalStateID` 算法；需要治理过滤时
通过 `/v1/query` 提供可信绑定的 `requester_agent_id`/roles。不要仅依赖相似度排序。
