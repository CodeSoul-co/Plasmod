...
| ~~R8~~ | ~~Knowhere C++ stub — two-layer gap (owner B)~~ | ✅ **FIXED** | B — **(1) C++ layer:** Real Knowhere HNSW/IVF/SPARSE implementation added with `#ifdef ANDB_USE_KNOWHERE` conditional compilation; stub fallback for dev/macOS. **(2) Go→Python bridge:** `HTTPRetrievalAdapter` in `src/internal/dataplane/http_retrieval_adapter.go` implements `DataPlane` interface via HTTP calls to Python retrieval service (`/ingest`, `/retrieve`, `/healthz` endpoints). No CGO or Milvus dependencies required. Tests in `http_retrieval_adapter_test.go`. |
...
**Remaining blockers for main merge:** All known issues (R1-R10) are now resolved. R8 was fixed by implementing a CogDB-native Go→Python HTTP bridge that bypasses CGO/Milvus dependencies entirely.
