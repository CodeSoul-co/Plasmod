...
- Go server ([`src/cmd/server/main.go`](src/cmd/server/main.go)) with **27 HTTP paths** registered in [`Gateway.RegisterRoutes`](src/internal/access/gateway.go) (see [HTTP API surface](#http-api-surface-v1)), graceful shutdown via `context.WithCancel`
- Admin dataset cleanup: `POST /v1/admin/dataset/delete` soft-deletes **Memory** records by dataset selectors (**AND** semantics) or by an explicit **`memory_ids`** list. **`workspace_id` is required.** Provide at least one of `file_name`, `dataset_name`, or `prefix`, **or** a non-empty `memory_ids` array (only those rows in the workspace are touched; selectors are ignored when `memory_ids` is set). `dry_run` only reports matches without mutating. Soft delete sets `IsActive=false` and evicts the hot-tier **cache** copy so stale rows are not served; **cold-tier embeddings are kept** until hard delete (`purge`) so metadata and vectors stay consistent. Query paths filter inactive memories.
...
|-------|-----------|
| **Health** | `GET /healthz` |
| **Admin** | `GET /v1/admin/topology` · `GET /v1/admin/storage` · `GET /v1/admin/config/effective` · `POST /v1/admin/s3/export` · `POST /v1/admin/s3/snapshot-export` · `POST /v1/admin/s3/cold-purge` · `POST /v1/admin/warm/prebuild` · `POST /v1/admin/dataset/delete` · `POST /v1/admin/dataset/purge` · `GET /v1/admin/dataset/purge/task` · `POST /v1/admin/memory/delete-by-source` · `POST /v1/admin/memory/purge-by-source` · `POST /v1/admin/data/wipe` · `POST /v1/admin/rollback` · `GET`/`POST /v1/admin/consistency-mode` · `POST /v1/admin/replay` |
